...
let forceify_file forceify game =
  ...
  (match forceify with
    Some(file) -> begin
      try
        let name,ext = split_resref (String.uppercase_ascii file) in
...
let biff_get bg_list game =
  ...
  let try_to_load str = begin
    try begin
      let base,ext = split_resref (String.uppercase_ascii str) in
...
let merge_tlk tlk_merge game =
  if_bgee_check_lang_or_fail game ;
  List.iter (fun str ->
    let name,ext = split_resref (String.uppercase_ascii str) in
...
let main () =
  ...
  let argv0_base, argv0_ext = split_resref
      (String.uppercase_ascii (Case_ins.filename_basename Sys.argv.(0))) in
  ...
  let auto () = begin
    ...
    if List.exists (fun arg ->
      let a,b = split_resref arg in (String.uppercase_ascii b) = "TP2")
