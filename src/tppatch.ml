  ...
  Stats.time "process_patch2" (fun () ->
    ...
    | TP_PatchReplaceBCSBlock(old_file, new_file, on_mismatch, eval, case_sensitive) -> begin
        ...
        let bcs_buff_of_baf_or_bcs file =
          let a,b = split_resref (String.uppercase_ascii file) in
          if b = "BAF" then begin
            try
...
(*           ( [] , [] ) *) in
    ...
    | TP_PatchStringEvaluate(case_sens,find, pl, replace) ->
        ...
        while not !finished do
          ...
          if not !finished then begin
            ...
            let tmp_buff = ref (Bytes.copy !work_buff) in
    ...
    | TP_Patch_Gam(cre_name,area,x,y) ->
        ...
        Bytes.blit (String.uppercase_ascii cre_name) 0 npc_entry_buff 0xc
    ...
    | TP_DecompileAndPatch pl ->
        let (dec,com) =
          let base,ext = split_resref (String.lowercase_ascii patch_filename) in
          match ext with
          | "bcs"
          | "bs" ->
    ...
    | TP_RefactorTrigger(pre,post,case_sens, exact_m, which) -> begin
        try
          ...
          let (fn,name) =
            ...
            | Some x -> "", x
            | None -> split_resref (String.lowercase_ascii patch_filename)
            in
            match ext with
            | "bs"
            | "baf"
